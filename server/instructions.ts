...
TALKING ON AN ISSUE:
Each issue can have a conversation of its own — post_to_issue writes there, and the user reads and answers it from the issue tracker without leaving it. Use it when what you have to say is ABOUT one issue rather than about what you are doing right now: a question that blocks the work, a finding, a proposal for how to proceed. Prefer it over raising the same thing on the general board — a running conversation interleaved with unrelated issue chatter stops being readable, which is the reason this exists. The thread is created by the FIRST message: post_to_issue makes the issue's board and node on demand — you never create a board yourself and then link it to the issue, post_to_issue IS the whole workflow. On that first call its response tells you it had to create the board and where it is; after that, keep using post_to_issue (it reuses the same board) or post_to_node on the board and node a reply carries. Every message posted there is linked to that issue automatically, so it also shows up in get_issue_timeline.
An issue's conversation lives on an ORDINARY board — it appears in list_boards and opens like any other; one issue, one board, a single serial thread until a discussion genuinely needs to branch (add a concern/node then). You never track its ids yourself: post_to_issue takes only the issue id, and a reply already carries its board and node. File work as issues WITHOUT being asked, too — when the user hands you something substantial or worth tracking, create_issue it first and hold the discussion on that issue via post_to_issue, so the reasoning is captured under the work instead of scattered on the general board. A trivial one-off needs no issue.
...
YOU CAN SEND IMAGES, not just receive them — and you almost certainly send too few. THE TRIGGER IS BEHAVIOURAL, not technical: the moment you catch yourself about to describe a SHAPE in prose — a before/after, a route or topology, a measured timeline, a branching procedure, relative magnitudes, a UI/icon mockup, a screenshot of what you built — stop and draw it instead. A markdown image \`![alt](/uploads/<board_id>/<file>.png)\` renders inline in the thread exactly like a user's pasted image, at whatever point in your text you place it, so the figure can sit right where the argument needs it. The user reviews a visual far faster than a paragraph describing one, especially for design/visual decisions. Do NOT draw when the point fits in one sentence or a markdown table already says it, and never let the picture carry the conclusion alone — write the takeaway in text too. FULL PROCEDURE — how to GENERATE the figure (matplotlib via \`uv run --with matplotlib\`, including the CJK font setting without which Japanese labels render as tofu), and when to use the mermaid diagram surface instead: the bundled skill **dt-visual-reply**. Short version: dt already serves \`~/.discussion-tree/uploads/<board_id>/\` on the web at \`/uploads/<board_id>/…\`, so you do NOT upload and you do NOT base64 anything — you write the PNG into that folder and link it. The file being in the folder IS the publish step.
  ...
  2. Embed that URL where it belongs in the post body: post_to_node(..., message: "...prose...\\n\\n![comparison](/uploads/<board_id>/agent_<unique>.png)"). Same for post_to_map_node / post_diagram_chat. (A CC instance can also screenshot an HTML/SVG render via a headless browser and drop that PNG in the same way.)
  e.g. (shell): \`DIR="\${DISCUSSION_TREE_HOME:-\$HOME/.discussion-tree}/uploads/<id>"; mkdir -p "\$DIR"; N="agent_\$(date +%s)_\$RANDOM.png"; cp fig.png "\$DIR/\$N"; echo "/uploads/<id>/\$N"\` → embed that path. (Off-box fallback only: POST base64 to /upload-image {board_id, filename, data_base64} → use the returned url; pipe base64 through the shell, never into your context.)
...
- set_node_status: Set an item's status (pending / discussing / resolved / agreed / adopted / rejected / needs-reply / done)
...
- list_boards / get_board / search_boards: read back past boards — see PAST DISCUSSIONS ARE QUERYABLE below for signatures and the recall pattern.
- reset_unanswered_posts: yield the whole unanswered set when you handled it another way — see UNANSWERED NODES below.
- report_bg_task_done / clear_bg_tasks: clear BG markers — see BACKGROUND TASK MARKER below (clearing is normally automatic; these are the fast-path and the reset).
...
The broker tracks which UI submissions you have NOT replied to yet — the "unanswered set". It spans all three surfaces, and the tool that CLEARS an entry differs per surface: a board node → post_to_node; a map's PER-NODE thread → post_to_map_node; a diagram chat → post_diagram_chat. (A map's general chat "__general__" is deliberately NOT tracked — there you answer by growing the graph, so a nag would over-fire.) In every case only a NON-EMPTY message clears it: a status-only post does NOT, and neither do set_node_status / record_decision / other tools. When your turn ends, a Stop hook NAMES anything still in the set — and names the tool to reply with — surfacing the case where you answered only in the CLI and forgot to mirror. Replying on a DIFFERENT node than the user asked on is legitimate — but the original node stays flagged, so if that was deliberate (you handled it elsewhere, or the user doesn't want a reply) call reset_unanswered_posts to clear the whole set and yield. The broker caps consecutive identical nags (MAX_NAG_STREAK) so a genuinely-stuck turn eventually yields and the user can step in.

DIAGRAMS (the 3rd dt surface, alongside boards & maps):
A "diagram" is dt's canonical name for this surface — ONE Mermaid source rendered on its own /diagram/:id page (flowchart / sequenceDiagram / classDiagram / stateDiagram / erDiagram / etc.). When the user says "diagram" (or the Japanese "daiaguramu") they mean exactly this surface — use the same word back; do NOT call it a "chart" or "graph". upsert_diagram creates or replaces a diagram rendered on its own /diagram/:id page — omit \`id\` to create, pass an existing \`id\` to replace the WHOLE source (no partial edits; ONE Mermaid diagram per source). Pass \`context\` to set a markdown description / background shown at the TOP of the page (what the diagram is, why it exists) — omit it on a source-only edit to keep the existing description, pass "" to clear it. get_diagram reads it back before you edit; list_diagrams / rename_diagram / archive_diagram (hide from the sidebar) / delete_diagram manage them. The broker rejects empty / non-Mermaid sources and the page shows any residual parse error. The page has a RIGHT-SIDE CHAT: when the user types there you receive a <channel kind="diagram_chat" ...> message whose board_id is the diagram's id — act on it by editing the diagram via upsert_diagram (the open page re-renders live on every upsert) and reply with post_diagram_chat(diagram_id, message) so your text shows in that chat thread. A diagram chat DOES count toward the unanswered-node nag (it is a question addressed to you), and only a post_diagram_chat with a real message clears it.
