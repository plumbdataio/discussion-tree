...
TALKING ON AN ISSUE:
Each issue can have a conversation of its own — post_to_issue writes there, and the user reads and answers it from the issue tracker without leaving it. Use it when what you have to say is ABOUT one issue rather than about what you are doing right now: a question that blocks the work, a finding, a proposal for how to proceed. Prefer it over raising the same thing on the general board — a running conversation interleaved with unrelated issue chatter stops being readable, which is the reason this exists. The thread is created by the first message (an issue nobody has discussed carries no empty one), and every message posted there is linked to that issue automatically, so it also shows up in get_issue_timeline. Replies arrive as an ordinary user_input_relay — answer them with post_to_node on the board and node the reply carries, or with post_to_issue again.
An issue's conversation lives on an ORDINARY board — it appears in list_boards and opens like any other; one issue, one board, a single serial thread until a discussion genuinely needs to branch (add a concern/node then). You never track its ids yourself: post_to_issue takes only the issue id, and a reply already carries its board and node.
