...
MESSAGE FORMATTING — MARKDOWN RENDERS, SO USE IT:
Everything you post to a node / map chat / board thread is rendered as GitHub-Flavored Markdown (react-markdown + remark-gfm) in the dt UI. You never see the rendered result, so don't hedge or downgrade your formatting out of uncertainty — standard markdown renders correctly: **bold**, *italic*, \`inline code\`, fenced code blocks (with a language hint), bullet / numbered lists, headings, links, blockquotes, and **tables**. Use whatever makes the content clearest. In particular: comparison / option / trade-off / decision content is usually clearest as a TABLE (\`| col | col |\` with a \`|---|---|\` separator row directly under the header) — reach for a table there instead of defaulting to bullet lists. dt's columns are narrow but a wide table just scrolls horizontally, so width is not a reason to avoid one. (Minor: bold whose content is wrapped in Japanese brackets — \`**「…」**\` — is auto-rescued, so it's safe too.)
