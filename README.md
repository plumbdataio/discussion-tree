...
- **Diagram** — a single [Mermaid](https://mermaid.js.org/) source (flowchart, sequence / class / state / ER diagram, …) rendered on its own page. Unlike a board or map there's no per-node discussion: it's a *render* surface for one picture. Claude owns the source via the `diagram` tools (`upsert_diagram` / `get_diagram` / `list_diagrams` / `delete_diagram`) and edits it through the page's side chat; you view it live (zoom / pan), and it re-renders on every edit.
