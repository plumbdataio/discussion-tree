...
- **Surfaces: boards, maps, diagrams — plus checklists.** A **board** is the converge-and-decide surface (a concern → item tree you settle one card at a time). A **map** is the diverge-and-explore surface that comes *before* a board (a free-form graph of cards you and Claude co-build spatially). A **diagram** is a single Mermaid source (flowchart, sequence diagram, etc.) rendered on its own page; Claude maintains it through the page's side chat and you view it live. A **checklist** is a node that tracks items with a status each (decisions, acceptance criteria, work steps) — Claude keeps it current. See [Concepts](#concepts) and [Using it](#using-it-the-standard-workflow).
...
Three more surfaces sit alongside boards:
...
- **Diagram** — a single [Mermaid](https://mermaid.js.org/) source (flowchart, sequence / class / state / ER diagram, …) rendered on its own page, with an optional markdown description at the top. Unlike a board or map there's no per-node discussion: it's a *render* surface for one picture. Claude owns the source — there's no user upload path — and you steer edits through the page's side chat; the page re-renders live on every change (zoom / pan to read it). Managed end-to-end via the `diagram` MCP tools (create / read / rename / archive / delete, plus the side chat).
...
- *"Draw that flow as a diagram."* → Claude renders a Mermaid diagram on its own page; you view it live (zoom / pan) and refine it through the page's side chat.
- *"Track those as a checklist."* → Claude marks a checklist node and keeps it current as decisions land.

Claude reads its own instructions and knows what discussion-tree can do — but it won't volunteer a tour, and it isn't responsible for teaching you the tool (it'll explain if you ask). So this is your half of the contract: knowing these surfaces exist and asking for them. Everything you create is just the conversation made persistent, structured, and revisitable.
